...
(* As pointed out by kirang, Link clause should really come first since
   it may be a link to an unbound variable. See an example below.
*)
...
let
 TArrow (TVar {contents = Link (TVar {contents = Unbound "d"})},
 TVar {contents = Unbound "d"})
 =
 reset_gensym (); 
...
try 
 reset_gensym (); 
 let _ = typeof [] (Lam ("x",App (Var"x",Var"x"))) in
 assert false
...
try 
 reset_gensym (); 
 let _ = ignore (typeof [] (Let ("x",Var"x",Var"x"))) in
 assert false
...
try 
 reset_gensym (); 
 let _ = 
  typeof [] (Lam ("y", App (Var"y",(Lam ("z", (App (Var"y", Var"z"))))))) in
 assert false
 with Failure e -> print_endline e
;;
      ...
      (TVar
        {contents =
          Link
           (TVar
             {contents =
               Link (TVar {contents = Link (TVar {contents = Unbound "i"})})})})},
 TArrow
  (TVar
    {contents = Link (TVar {contents = Link (TVar {contents = Unbound "i"})})},
  TArrow
   (TVar
     {contents =
       Link
                   ...
                   Link
                    (TVar
                      {contents =
                        Link
                         (TVar
                           {contents = Link (TVar {contents = Unbound "i"})})})})},
          TVar
           {contents =
             Link
              (TArrow
                (TVar
                  {contents =
                    Link
                     (TVar {contents = Link (TVar {contents = Unbound "i"})})},
                TVar
                 {contents =
                   Link
                    (TVar
                      {contents =
                        Link
                         (TVar
                           {contents = Link (TVar {contents = Unbound "i"})})})}))}))},
   TVar {contents = Unbound "i"})))
 =
 reset_gensym (); 
 typeof [] (Lam ("x", Lam ("y", Lam ("k",
